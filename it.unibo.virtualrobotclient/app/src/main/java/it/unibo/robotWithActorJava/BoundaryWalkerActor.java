...
public class BoundaryWalkerActor extends ActorBasicJava {
    ...
    String robotCmdStop       = "{\"robotcmd\":\"STOP\" }";
    String robotCmdResume     = "{\"robotcmd\":\"RESUME\" }";

    private enum State {start, walking, obstacle, idle, end };
    private IssWsHttpJavaSupport support;
    private State curState       =  State.start ;

    private State previousState = null;//I record the previous state before IDLE.
        ...
        switch( curState ) {
            ...
            case idle:
                if( move.equals("RESUME") ){
                    curState = (previousState == State.walking) ? State.walking : State.obstacle;
                }//else ignore the STOP command.
                break;
            case end : {
